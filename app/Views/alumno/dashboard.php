...
<nav class="bg-slate-50 dark:bg-slate-950 font-manrope font-medium h-screen w-64 fixed left-0 top-0 no-border shadow-right shadow-[4px_0_24px_rgba(6,105,114,0.04)] hidden lg:flex flex-col py-6 z-40">
    ...
    <div class="px-4 mb-6">
        <button onclick="resetForm()" class="w-full bg-primary text-on-primary rounded-full py-3 px-4 flex items-center justify-center gap-2 shadow-sm shadow-primary/20 hover:opacity-90 transition-opacity">
            ...
            <span class="font-semibold text-sm">Nuevo Reporte</span>
        </button>
    </div>
...
<main class="flex-1 w-full lg:ml-64 flex flex-col min-h-screen pb-24 lg:pb-0">
    <div class="px-6 py-10 lg:px-margin-page lg:py-12 max-w-6xl mx-auto w-full flex-1 flex flex-col gap-stack-gap">
        <header class="mb-4">
            <h2 class="font-h1 text-h1 text-primary">Espacio Seguro</h2>
            <p class="font-body-lg text-body-lg text-on-surface-variant mt-2 max-w-2xl">Un lugar tranquilo para compartir lo que te preocupa. Tu voz es importante y te escucharemos sin juzgar.</p>
        </header>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-gutter items-start">
                    ...
                    <div class="relative z-10">
                        ...
                        <div class="flex items-center justify-between mb-8" id="progress-header">
                            <div class="flex items-center gap-3">
                                ...
                                <span id="label-1" class="font-body-md text-body-md text-primary font-medium transition-colors">¿Qué pasó?</span>
                            </div>
                        ...
                        <div id="form-container">
                            ...
                            <div id="step-1" class="space-y-6">
                                <div>
                                    <label class="block font-h2 text-h2 text-on-surface mb-2">Cuéntanos qué sucedió</label>
                                    <p class="font-body-md text-body-md text-on-surface-variant mb-4">Tómate tu tiempo. Escribe todo lo que consideres importante.</p>
                                    <textarea id="report-content" class="w-full h-48 bg-surface-container-highest rounded-lg p-4 font-body-md text-body-md text-on-surface placeholder:text-outline border-0 focus:ring-0 focus:shadow-[0_0_15px_rgba(6,105,114,0.15)] transition-shadow resize-none" placeholder="Empezó cuando..."></textarea>
                                    <p id="error-step-1" class="text-error text-sm mt-2 hidden">Por favor, escribe al menos 5 caracteres.</p>
                                </div>
                            ...
                            <div id="step-2" class="space-y-6 hidden">
                                <div class="space-y-3">
                                    <p class="font-body-md text-[14px] text-on-surface font-medium">¿A quién le ha pasado?</p>
                                    <div class="flex flex-wrap gap-2">
                                        <button type="button" class="target-btn px-4 py-2 rounded-full border border-primary bg-primary-fixed/20 text-on-primary-fixed-variant text-sm font-medium transition-colors" data-value="yo_mismo" onclick="setTarget(this)">Me ha pasado a mí</button>
                                        <button type="button" class="target-btn px-4 py-2 rounded-full border border-outline-variant bg-transparent text-on-surface-variant hover:bg-surface-variant text-sm font-medium transition-colors" data-value="otro" onclick="setTarget(this)">A otra persona</button>
                                    </div>
                                    <input type="hidden" id="report-target" value="yo_mismo">
                                </div>

                                <div class="space-y-3 pt-2">
                                    <p class="font-body-md text-[14px] text-on-surface font-medium">Nivel de prioridad</p>
                                    <div class="flex flex-wrap gap-2">
                                        <button type="button" class="urgency-btn px-4 py-2 rounded-full border border-primary bg-primary-fixed/20 text-on-primary-fixed-variant text-sm font-medium transition-colors" data-value="low" onclick="setUrgency(this)">Baja Prioridad</button>
                                        <button type="button" class="urgency-btn px-4 py-2 rounded-full border border-outline-variant bg-transparent text-on-surface-variant hover:bg-surface-variant text-sm font-medium transition-colors" data-value="medium" onclick="setUrgency(this)">Seguimiento</button>
                                        <button type="button" class="urgency-btn px-4 py-2 rounded-full border border-outline-variant bg-transparent text-on-surface-variant hover:bg-surface-variant text-sm font-medium transition-colors" data-value="high" onclick="setUrgency(this)">Alta Prioridad</button>
                                    </div>
                                ...
                                <div class="bg-surface rounded-lg p-4 flex items-start gap-4 shadow-sm shadow-primary/5 mt-4 border border-surface-variant">
                                    ...
                                    <div>
                                        <label class="font-body-lg text-body-lg text-on-surface font-medium cursor-pointer" for="report-anonymous">Enviar de forma anónima</label>
                                        <p class="font-body-md text-[14px] text-on-surface-variant mt-1">Tu identidad no será revelada a profesores. Solo el equipo de soporte sabrá quién eres para poder ayudarte.</p>
                                    </div>
                                </div>

                                <div class="flex justify-between pt-4">
                                    <button type="button" onclick="prevStep(1)" class="bg-surface-container text-on-surface-variant rounded-full px-6 py-3 font-body-md text-body-md font-medium hover:bg-surface-variant transition-colors">Atrás</button>
                                    <button type="button" id="btn-submit-report" onclick="submitReport()" class="bg-primary text-on-primary rounded-full px-8 py-3 font-body-md text-body-md font-medium shadow-[0_4px_14px_rgba(0,79,86,0.2)] hover:shadow-[0_6px_20px_rgba(0,79,86,0.3)] hover:-translate-y-0.5 transition-all flex items-center gap-2">
                                        Enviar Mensaje Seguro <span class="material-symbols-outlined text-sm">send</span>
                                    </button>
                                </div>
                            </div>
                            ...
                            <div id="step-success" class="text-center py-12 hidden flex-col items-center gap-4">
                                ...
                                <h3 class="font-h2 text-h2 text-on-surface">Recibido con éxito</h3>
                                <p class="font-body-md text-body-md text-on-surface-variant max-w-sm">Gracias por confiar en nosotros. Tu mensaje ya está en buenas manos.</p>
                                <button type="button" onclick="resetForm()" class="mt-4 bg-surface-container text-on-surface-variant rounded-full px-8 py-3 font-body-md text-body-md font-medium hover:bg-surface-variant transition-colors">Volver a mi panel</button>
                            </div>
                            ...
                            <div id="chat-view" class="hidden flex-col min-h-[500px]">
                                <div class="flex items-center justify-between border-b border-surface-variant pb-4 mb-4">
                                    <button onclick="resetForm()" class="flex items-center text-primary font-medium text-sm hover:underline"><span class="material-symbols-outlined text-sm mr-1">arrow_back</span> Volver al menú</button>
                                ...
                                <div id="chat-input-container" class="relative">
                                    <input id="reply-message" class="w-full bg-surface-container border-0 rounded-full py-3 pl-4 pr-12 font-body-md text-[14px] text-on-surface focus:ring-0 transition-shadow" placeholder="Escribe un mensaje..." type="text"/>
            ...
            <div class="lg:col-span-4 flex flex-col gap-6">
                <div class="bg-surface-container-lowest rounded-xl shadow-[0_8px_40px_rgba(0,79,86,0.04)] p-card-padding">
                    <h3 class="font-h2 text-[20px] text-on-surface mb-6 flex items-center gap-2"><span class="material-symbols-outlined text-primary">history</span> Tus Avisos</h3>
                    <div class="space-y-4">
                        <?php if (empty($reports)): ?><div class="p-6 text-center text-outline text-sm italic">Sin actividad reciente</div><?php else: ?>
                ...
                <div class="bg-secondary-container rounded-xl shadow-[0_8px_40px_rgba(0,79,86,0.04)] p-card-padding flex flex-col items-center text-center">
                    ...
                    <h4 class="font-body-lg text-[18px] font-semibold text-on-secondary-container mb-2">¿Necesitas hablar ahora?</h4>
                    <button class="bg-surface-container-lowest text-secondary rounded-full px-6 py-2 font-body-md text-body-md font-medium shadow-sm hover:shadow-md transition-shadow">Chat de Ayuda</button>
                </div>
            </div>
        </div>
    </div>
...
<div id="breathing-app-container" class="fixed inset-0 z-[100] hidden flex-col items-center justify-center overflow-hidden" style="font-family: 'Outfit', sans-serif; color: #f4ede4; background: #1a2f3a;">
    ...
    <section id="b-landing" class="absolute inset-0 flex items-center justify-center p-8 transition-all duration-700 z-[106]" style="background: radial-gradient(ellipse at 20% 20%, rgba(168, 197, 181, 0.22) 0%, transparent 55%), radial-gradient(ellipse at 80% 90%, rgba(197, 217, 229, 0.18) 0%, transparent 60%), linear-gradient(165deg, #0f1e26 0%, #1a2f3a 50%, #2d5160 100%);">
        <div class="text-center max-w-lg">
            <p class="text-[0.7rem] uppercase tracking-[0.3em] text-[#a8c5b5] mb-6">Bienestar · Pausa consciente</p>
            ...
            <p class="text-[#c5d9e5] opacity-80 mb-10 text-lg">Una pausa breve para volver a ti mismo.</p>
            <div class="flex flex-wrap gap-2 justify-center mb-10">
                <button onclick="selectRhythm('calm', this)" class="r-btn active px-4 py-2 rounded-full border border-white/20 bg-white/5 text-[#c5d9e5] text-sm hover:bg-white/10 transition-all">Calma<small class="block opacity-60 text-[10px]">4 · 6</small></button>
                <button onclick="selectRhythm('box', this)" class="r-btn px-4 py-2 rounded-full border border-white/20 bg-white/5 text-[#c5d9e5] text-sm hover:bg-white/10 transition-all">Enfoque<small class="block opacity-60 text-[10px]">4 · 4 · 4 · 4</small></button>
                <button onclick="selectRhythm('sleep', this)" class="r-btn px-4 py-2 rounded-full border border-white/20 bg-white/5 text-[#c5d9e5] text-sm hover:bg-white/10 transition-all">Descanso<small class="block opacity-60 text-[10px]">4 · 7 · 8</small></button>
            </div>
            <button onclick="startSession()" class="bg-[#f4ede4] text-[#0f1e26] px-12 py-4 rounded-full font-bold text-lg shadow-2xl hover:translate-y-[-2px] transition-all">Respira Conmigo</button>
        </div>
    </section>

    <section id="b-scene" class="hidden absolute inset-0 flex items-center justify-center transition-all duration-1000 z-[107]">
        <div class="absolute top-10 left-1/2 -translate-x-1/2 text-[0.7rem] tracking-[0.4em] opacity-40 uppercase">Ciclo <strong id="b-cycle" class="text-[#a8c5b5]">1</strong></div>
        <div class="relative flex items-center justify-center w-[300px] h-[300px] md:w-[500px] md:h-[500px]">
            ...
            <div id="b-circle" class="relative w-3/5 h-3/5 rounded-full flex flex-col items-center justify-center text-center shadow-2xl transition-all ease-in-out" style="background: radial-gradient(circle at 35% 30%, rgba(244, 237, 228, 0.25) 0%, rgba(168, 197, 181, 0.4) 40%, rgba(45, 81, 96, 0.6) 100%);">
                <div id="b-label" class="text-3xl md:text-5xl font-light italic" style="font-family: 'Fraunces', serif;">Prepárate</div>
                <div id="b-timer" class="text-xs opacity-50 mt-2 tracking-widest"></div>
            </div>
        </div>
        <p class="absolute bottom-12 uppercase tracking-[0.4em] text-[0.6rem] opacity-40">Sigue el ritmo · respira despacio</p>
    </section>
</div>
...
<script>
    ...
    const LABELS = ['Inhala', 'Mantén', 'Exhala', 'Mantén'];
