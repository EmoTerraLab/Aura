...
<main class="min-h-screen bg-slate-50 py-10 px-4 md:px-10">
    <div class="max-w-7xl mx-auto space-y-10">
        ...
        <header class="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
                <h1 class="text-3xl font-black text-slate-800 uppercase tracking-tight"><?= Lang::t('protocol.dashboard_title') ?></h1>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-2">
                <p class="text-[10px] font-black uppercase text-slate-400 tracking-widest"><?= Lang::t('protocol.active_expedients') ?></p>
            ...
            <div class="bg-red-600 p-8 rounded-[2.5rem] text-white shadow-xl shadow-red-600/20 space-y-2">
                <p class="text-[10px] font-black uppercase opacity-60 tracking-widest"><?= Lang::t('protocol.barnahus_alerts') ?></p>
                <div class="flex items-end gap-3">
            ...
            <div class="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-2">
                <p class="text-[10px] font-black uppercase text-slate-400 tracking-widest"><?= Lang::t('protocol.weekly_followups') ?></p>
                <div class="flex items-end gap-3">
        ...
        <div class="bg-amber-50 border border-amber-200 p-6 rounded-3xl space-y-3">
            <h3 class="text-amber-800 font-black text-xs uppercase flex items-center gap-2">
                <span class="material-symbols-outlined text-sm">warning</span> <?= Lang::t('protocol.deadline_alerts') ?>
            </h3>
            <ul class="space-y-1">
        ...
        <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
            <div class="p-8 border-b border-slate-50 flex justify-between items-center">
                <h2 class="font-black text-slate-800 uppercase text-sm tracking-widest"><?= Lang::t('protocol.active_cases_table') ?></h2>
            </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50/50 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                            ...
                            <th><?= Lang::t('protocol.student') ?></th>
                            <th><?= Lang::t('protocol.phase') ?></th>
                            <th><?= Lang::t('protocol.days_active') ?></th>
                    ...
                    <tbody class="text-sm">
                        ...
                        <tr class="border-b border-slate-50 hover:bg-slate-50/30 transition-colors group">
                            ...
                            <td>
                                <p class="font-black text-slate-800"><?= htmlspecialchars($case['student_name']) ?></p>
                                <p class="text-[10px] font-bold text-slate-400 uppercase"><?= htmlspecialchars($case['classroom_name']) ?></p>
                            </td>
                            <td>
                                <?php 
                                    $phaseColors = [
                            ...
                            <td class="font-bold text-slate-600"><?= floor($case['days_active']) ?> <?= Lang::t('protocol.days') ?></td>
                            <td class="px-8">
                        ...
                        <?php if (empty($cases)): ?>
                        <tr>
                            <td colspan="5" class="py-20 text-center text-slate-400 italic"><?= Lang::t('protocol.no_active_cases') ?></td>
                        </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
