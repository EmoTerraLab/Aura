...
<nav class="bg-slate-50 dark:bg-slate-950 shadow-[4px_0_24px_rgba(6,105,114,0.04)] hidden lg:flex flex-col py-6 h-screen w-64 fixed left-0 top-0 z-40">
    ...
    <div class="flex-1 overflow-y-auto no-scrollbar space-y-1">
        <a class="flex items-center gap-3 bg-teal-50 dark:bg-teal-900/20 text-teal-700 dark:text-teal-300 rounded-full mx-2 px-4 py-3 transition-colors scale-95 duration-150" href="/staff/inbox"><span class="material-symbols-outlined" style="font-variation-settings: 'FILL' 1;">inbox</span><span class="font-body-md text-body-md font-medium">Bandeja de Entrada</span></a>
        <?php if (\App\Core\Auth::role() === 'admin'): ?><a class="flex items-center gap-3 text-slate-500 dark:text-slate-400 px-4 py-3 mx-2 hover:bg-teal-50/50 dark:hover:bg-teal-900/10 rounded-full transition-colors" href="/admin"><span class="material-symbols-outlined">admin_panel_settings</span><span class="font-body-md text-body-md font-medium">Panel Admin</span></a><?php endif; ?>
        <a class="flex items-center gap-3 text-slate-500 dark:text-slate-400 px-4 py-3 mx-2 hover:bg-teal-50/50 dark:hover:bg-teal-900/10 rounded-full transition-colors" href="#"><span class="material-symbols-outlined">folder_open</span><span class="font-body-md text-body-md font-medium">Casos Activos</span></a>
        <div class="mt-8 px-4"><div class="bg-secondary-container rounded-DEFAULT p-4 ambient-shadow relative overflow-hidden"><div class="absolute -right-4 -top-4 w-16 h-16 bg-white/20 rounded-full blur-xl"></div><span class="material-symbols-outlined text-secondary mb-2">hub</span><h3 class="font-body-md text-body-md font-semibold text-on-secondary-container leading-tight">Sociogramas Automáticos</h3><p class="font-label-caps text-label-caps text-secondary mt-1 normal-case">Dinámicas ocultas</p></div></div>
    </div>
...
<div id="mentions-dropdown" class="hidden fixed lg:absolute right-4 top-16 lg:top-20 mt-2 w-80 rounded-xl shadow-lg bg-surface-container-lowest ring-1 ring-black ring-opacity-5 z-[60] ambient-shadow overflow-hidden">
    <div class="p-4 bg-surface-container-low border-b border-surface-variant flex justify-between items-center"><h3 class="text-sm font-bold text-on-surface">Menciones</h3></div>
...
<main class="flex-1 lg:ml-64 flex flex-col md:flex-row h-screen pt-16 lg:pt-0 bg-surface">
    ...
    <section class="w-full md:w-[35%] lg:w-[30%] h-full flex flex-col bg-surface-container-lowest ambient-shadow z-10 overflow-hidden">
        <div class="p-6 pb-2">
            <h2 class="font-h2 text-h2 text-on-surface mb-4">Bandeja de Entrada</h2>
            <div class="flex gap-2 mb-4 overflow-x-auto no-scrollbar pb-2">
                <button class="whitespace-nowrap px-4 py-1.5 rounded-full bg-primary-container text-on-primary-container font-label-caps text-label-caps tracking-wide">Todos</button>
            </div>
        </div>
            ...
            <?php foreach($reports as $report): ?>
                ...
                <div class="bg-surface-container-lowest hover:bg-surface border-l-4 <?= $report['status']==='new'?'border-primary':'border-transparent' ?> p-4 rounded-DEFAULT cursor-pointer transition-colors" onclick="loadReport(<?= $report['id'] ?>)">
                    <div class="flex justify-between items-start mb-2">
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full <?= $badge ?> font-label-caps text-[10px] tracking-wider uppercase font-bold">Aula <?= htmlspecialchars($report['classroom_name']) ?></span>
    ...
    <section class="flex-1 h-full flex flex-col bg-surface-bright hidden md:flex relative" id="report-detail-container">
        <div class="flex-1 flex flex-col items-center justify-center text-outline">
            ...
            <h3 class="font-h2 text-[20px] font-medium text-on-surface-variant">Selecciona un aviso</h3>
        </div>
    </section>
</main>
...
<nav class="lg:hidden fixed bottom-0 w-full z-50 flex justify-around items-center px-4 pb-6 pt-3 bg-white/90 backdrop-blur-lg shadow-lg rounded-t-[32px]">
    <a class="flex flex-col items-center text-slate-400" href="#"><span class="material-symbols-outlined">home</span><span class="text-[11px]">Inicio</span></a>
...
<div id="premium-modal" class="fixed inset-0 z-[100] hidden bg-black/50 backdrop-blur-sm flex items-center justify-center">
    <div class="bg-white rounded-xl p-8 text-center max-w-md m-4">
        <h3 class="font-h1 text-[24px] font-bold mb-2">Aura Premium</h3>
        <p class="mb-8">Análisis IA ELOS.</p>
        <button onclick="closePremiumModal()" class="w-full py-3 bg-primary text-white rounded-full mb-2">Contactar</button>
        <button onclick="closePremiumModal()" class="w-full py-3 text-slate-500">Cerrar</button>
    </div>
</div>
...
<script>
        ...
        let mHtml = messages.map(m => {
            ...
            return `<div class="flex gap-3 ${isMe?'flex-row-reverse':''} mb-4">
                ...
                <div class="${isInt?'bg-amber-50':(isMe?'bg-primary text-white':'bg-white border')} p-3 rounded-2xl ${isMe?'rounded-tr-none':'rounded-tl-none'} shadow-sm max-w-[85%]">
                    ${isInt?'<p class="text-[9px] font-black text-amber-800 mb-1">🔒 NOTA INTERNA</p>':''}
        ...
        container.innerHTML = `
            <div class="h-20 px-8 flex items-center justify-between bg-white border-b z-20">
                <div class="flex items-center gap-4">
                    ...
                    <div><h3 class="font-bold text-sm">Caso #${report.id}</h3><p class="text-[10px] text-outline uppercase font-bold">Aula ${report.classroom_name}</p></div>
                </div>
                <div class="flex gap-2">
                    <button onclick="showPremiumModal()" class="bg-slate-100 text-slate-700 px-3 py-1.5 rounded-full text-[10px] font-black uppercase">IA ELOS</button>
                    <select onchange="handleStatusChange(${report.id}, this.value)" id="status-select" class="bg-slate-100 border-0 rounded-full py-1.5 px-4 text-[10px] font-black uppercase">
                        <option value="new" ${report.status==='new'?'selected':''}>Recibido</option>
                        <option value="in_progress" ${report.status==='in_progress'?'selected':''}>En Revisión</option>
            ...
            <div class="flex-1 overflow-y-auto p-6 space-y-6 bg-slate-50">
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
                    <h4 class="text-xs font-black uppercase text-slate-400 mb-3 tracking-widest">Relato del Alumno</h4>
                    <p class="text-sm text-slate-800 whitespace-pre-wrap">${report.content}</p>
                </div>
                <div id="messages-flow">${mHtml || '<p class="text-center text-slate-400 py-10 text-xs italic">Aún no hay respuestas en este hilo.</p>'}</div>
            </div>
            <div class="p-4 bg-white border-t">
                <div class="flex gap-2 mb-3">
                    <input id="reply-message" class="flex-1 bg-slate-100 border-0 rounded-full py-3 px-6 text-sm" placeholder="Escribe un mensaje..."/>
                ...
                <label class="text-[11px] font-bold text-slate-500 flex items-center gap-2 pl-4 cursor-pointer">
                    <input type="checkbox" id="reply-internal" class="rounded text-primary"/> Marcar como nota interna (solo staff)
                </label>
            </div>
        `;
    }
    ...
    async function handleStatusChange(id, status) {
        if (status === 'resolved') {
            const sum = prompt("Escribe el resumen de resolución:");
