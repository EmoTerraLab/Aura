...
<main class="flex-1 lg:ml-64 flex flex-col h-screen pt-16 lg:pt-0 bg-surface overflow-y-auto no-scrollbar">
    <div class="p-6 lg:p-10 max-w-5xl mx-auto w-full space-y-6">
        <header class="flex items-center justify-between">
            <h1 class="text-3xl font-black text-primary">Ajustes del Sistema</h1>
        </header>
        ...
        <?php if (isset($_GET['error'])): ?>
            <div class="bg-red-100 border border-red-200 text-red-800 px-4 py-3 rounded-xl flex items-center gap-2">
                <span class="material-symbols-outlined">error</span>
                <span class="text-sm font-bold">Ocurrió un error (<?= htmlspecialchars($_GET['msg'] ?? $_GET['error']) ?>).</span>
            </div>
        <?php endif; ?>
        <?php if (isset($_GET['test'])): ?>
            <div class="bg-blue-100 border border-blue-200 text-blue-800 px-4 py-3 rounded-xl flex items-center gap-2">
                <span class="material-symbols-outlined">info</span>
                <span class="text-sm font-bold">
                    <?php if($_GET['test'] === 'ok'): ?>
                        Correo de prueba enviado. Revisa la bandeja de entrada.
                    <?php else: ?>
                        Error al enviar correo: <?= htmlspecialchars($_GET['msg'] ?? '') ?>
                    <?php endif; ?>
                </span>
            </div>
        <?php endif; ?>
        ...
        <div class="flex gap-2 overflow-x-auto no-scrollbar pb-2">
            <a href="/admin/settings?tab=school" class="px-5 py-2.5 rounded-full text-sm font-bold whitespace-nowrap transition-colors <?= $tab === 'school' ? 'bg-primary text-white shadow-md' : 'bg-surface-container-lowest text-slate-500 hover:bg-surface-container-low' ?>">
                Escuela e Identidad
            </a>
            <a href="/admin/settings?tab=appearance" class="px-5 py-2.5 rounded-full text-sm font-bold whitespace-nowrap transition-colors <?= $tab === 'appearance' ? 'bg-primary text-white shadow-md' : 'bg-surface-container-lowest text-slate-500 hover:bg-surface-container-low' ?>">
                Apariencia
            </a>
            <a href="/admin/settings?tab=mail" class="px-5 py-2.5 rounded-full text-sm font-bold whitespace-nowrap transition-colors <?= $tab === 'mail' ? 'bg-primary text-white shadow-md' : 'bg-surface-container-lowest text-slate-500 hover:bg-surface-container-low' ?>">
                Correo (SMTP)
            </a>
            <a href="/admin/settings?tab=security" class="px-5 py-2.5 rounded-full text-sm font-bold whitespace-nowrap transition-colors <?= $tab === 'security' ? 'bg-primary text-white shadow-md' : 'bg-surface-container-lowest text-slate-500 hover:bg-surface-container-low' ?>">
                Seguridad y Autenticación
            </a>
        </div>

        <div class="bg-surface-container-lowest rounded-3xl border border-surface-variant/50 ambient-shadow p-6 lg:p-8">
            <?php
