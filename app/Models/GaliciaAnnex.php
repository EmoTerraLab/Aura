...
class GaliciaAnnex extends Model {

    protected $table = 'galicia_protocol_annexes';
    ...
    public function findByCase(int $caseId): array {
        $stmt = $this->db->prepare(
            "SELECT * FROM {$this->table} WHERE protocol_case_id = ? ORDER BY created_at ASC"
        );
        $stmt->execute([$caseId]);
    ...
    public function findLatestByType(int $caseId, string $type): ?array {
        $stmt = $this->db->prepare(
            "SELECT * FROM {$this->table} WHERE protocol_case_id = ? AND annex_type = ? ORDER BY created_at DESC LIMIT 1"
        );
